...
def build_task_specs(test_mode: bool = False) -> dict[str, TaskSpec]:
    ...
    for report_id in JYCM_REPORTS:
        ...
        specs[task_id] = TaskSpec(task_id, deps=(), resources=("browser_downloads",), runner=runner(task_id))
    ...
    for source in QUICKBI_SOURCES:
        ...
        # 无前置：QuickBI API 与 Step1 千牛/直播完全独立
        specs[f"quickbi_api.{source}"] = TaskSpec(
            f"quickbi_api.{source}", deps=(), resources=("quickbi_crawler",), runner=runner(f"quickbi_api.{source}")
        )
    for source in QUICKBI_SOURCES:
